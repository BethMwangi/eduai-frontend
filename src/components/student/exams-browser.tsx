          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Practice with official KCSE, KCPE papers and mock exams from top
                schools
              </p>
            </div>
            <div className="hidden md:flex items-center gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-accent">
                  {exams.reduce(
                    (sum, exam) =>
                      sum + (exam.papers.length || 0),
                    0
                  )}
                </div>
                <div className="text-sm text-gray-500">Papers Available</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {examTypes.map((type) => (
            <button
              key={type.key}
              onClick={() =>
                setSelectedType(selectedType === type.key ? "all" : type.key)
              }
            ...
            >
              ...
              <div className="text-xs text-gray-500">
                {exams.filter((e) => e.exam_type === type.key).length} available
              </div>
            </button>
          ))}
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex gap-4">
              <select
                ...
                onChange={(e) =>
                  setSelectedLevel(
                    e.target.value as "all" | "JUNIOR" | "SENIOR"
                  )
                }
